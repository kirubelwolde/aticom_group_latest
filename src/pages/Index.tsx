...
import { useBathroomInstallations } from "@/hooks/useBathroomSolutions";

const Index = () => {
  const [currentSlide, setCurrentSlide] = React.useState(0);
  const [isAutoPlaying, setIsAutoPlaying] = React.useState(true);
  const { data: installations } = useBathroomInstallations(true);
  ...
  // Build slides dynamically from installations, with a fallback to static slides
  const slides = React.useMemo(() => {
    const dynamicSlides = (installations || [])
      .filter((i) => !!i.image_url)
      .map((i) => ({
        image: i.image_url as string,
        title: i.title,
        subtitle: i.location || i.description || "",
      }));
    if (dynamicSlides.length > 0) return dynamicSlides;
    // Fallback to original static slides if no data
    return [
      {
        image:
              ...
              {slides.map((_, i) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-200 ${
                    currentSlide === i ? 'bg-white scale-125' : 'bg-white/60 hover:bg-white/80'
                  }`}
                  onClick={() => showSlide(i)}
                ></button>
              ))}
